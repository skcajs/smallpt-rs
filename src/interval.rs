...
// const G: f64 = 1e-4; //6.6743e-11;
// const M: f64 = 2.5;
const R: f64 = 5.;

const RS: f64 = 0.008; // 0.0001
...
pub fn schwarszchild(ray: &Ray, h: f64) -> Ray {
    ...
    let k2p = fp(previous_momentum + k1p * 0.5 * h, previous_point + k1x * 0.5 * h);

    let k3x = fx(previous_momentum + k2p * 0.5 * h);
    let k3p = fp(previous_momentum + k2p * 0.5 * h, previous_point + k2x * 0.5 * h);
    ...
    Ray {o: current_point, d: current_momentum}
}
...
pub fn fp(p: Tup, x:Tup) -> Tup {
    return x * (-1. / (2. * R.pow(3))) * (((p.0.pow(2) + p.1.pow(2) + p.2.pow(2)) * FACT_P1) + FACT_P2 ) * RS;
}